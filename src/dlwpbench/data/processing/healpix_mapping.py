...
class HEALPixRemap():
    ...
    def remap(
        ...
        coords = {}
        if not const:
            print("ADDING 11 HOURS")
            if "time" in list(ds_ll.coords): coords["time"] = ds_ll.coords["time"]  #+ pd.Timedelta(hours=11)
             # add 11 hourse
...
def project_ll_to_hpx(
    ...
    file_paths_ll = np.sort(glob.glob(os.path.join('/projects/prjs1254/ERA5_5.625/ERA5_1.0', vname, f"*.{extension}")))
    ...
    #file_paths_ll = np.sort(glob.glob(os.path.join("/projects/prjs1254/", vname, f"*.{extension}")))
    deg = 2.0 #5.625 #2.0 #1.0 #

    # /projects/prjs1254/ERA5_5.625/ERA5_1.0/
...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--variable-name", type=str, default="msl",
    ...
    parser.add_argument("-n", "--nside", type=int, default=32, #8
