...
class WeatherBenchDataset(th.utils.data.Dataset):
    ...
    def __init__(
        ...
        half_manual_32 = {"tisr": 
            {"file_name": "toa_incident_solar_radiation",
            "mean": 0.2501247227191925,
            "std": 0.322971373796463},
            ...
            {"file_name": "z-700",
            "mean": 29778.04296875,
            "std": 1729.84716796875},

            "z-850": 
            {"file_name": "z-850",
            "mean": 14256.275390625,
            "std": 1190.5186767578125},

            "z-925": 
            {"file_name": "z-925",
            "mean": 7366.966796875,
            "std": 999.531005859375},

            "z-1000": 
            {"file_name": "z-1000",
            "mean": 940.7884521484375,
            "std": 880.9417114257812},

            "q-850": 
            {"file_name": "q-850",
            "mean": 0.006182440556585789,
            "std": 0.004139884375035763},

            "q-500": 
            {"file_name": "q-500",
            "mean": 0.0011766536626964808,
            "std": 0.0012324426788836718},

            "2t": 
            {"file_name": "2t",
            "mean": 288.05322265625,
            "std": 14.82728385925293},

            "t-850": 
            {"file_name": "t-850",
            "mean": 281.70452880859375,
            "std": 12.062122344970703},
            ...
            {"file_name": "geopotential-150",
             "mean": 6.438661785068689e-06,
            "std": 3.903362085111439e-06},

            "geopotential-200": 
            {"file_name": "geopotential-200",
            "mean": 2.600587140477728e-05,
            "std": 2.3466101993108168e-05},

            "geopotential-250": 
            {"file_name": "geopotential-250",
            "mean": 7.74442742113024e-05,
            "std": 7.752280362183228e-05},

            "geopotential-300": 
            {"file_name": "geopotential-300",
            "mean": 0.00016896765737328678,
            "std": 0.00017708996892906725},

            "geopotential-400": 
            {"file_name": "geopotential-400",
            "mean": 0.0005014513153582811,
            "std": 0.0005381880328059196},

            "geopotential-500": 
            {"file_name": "geopotential-500",
             "mean": 0.0011052259942516685,
            "std": 0.0011521409032866359},

            "geopotential-600": 
            {"file_name": "geopotential-600",
            "mean": 0.0020001574885100126,
            "std": 0.001866795471869409},

            "geopotential-700": 
            {"file_name": "geopotential-700",
            "mean": 0.003169204108417034,
            "std": 0.002634126227349043},

            "geopotential-850": 
            {"file_name": "geopotential-850",
            "mean": 0.006028688512742519,
            "std": 0.004066576715558767},

            "geopotential-925": 
            {"file_name": "geopotential-925",
            "mean": 0.008005477488040924,
            "std": 0.004919120576232672},
            
            "geopotential-1000": 
            {"file_name": "geopotential-1000",
            "mean": 0.009385651908814907,
            "std": 0.00574483722448349},
        ...
        for p in prognostic_variable_names_and_levels:
            prog = glob.glob(os.path.join(data_path, self.stats[p]["file_name"], "*zarr*"))
            fpaths += prog

        for p in prescribed_variable_names:
            pres = glob.glob(os.path.join(data_path, self.stats[p]["file_name"], "*zarr*"))
            fpaths += pres

        if constant_names: 
            const = glob.glob(os.path.join(data_path, "constants", "*zarr*"))
            fpaths += const
            
        print(f"\tLoading dataset from {start_date} to {stop_date} into RAM...", sep=" ", end=" ", flush=True)
        a = time.time()
        # Initialize a variable to store the reference latitude
        ...
        # Issues when loading the entire dataset
        ...
        #self.statistics = None self.compute_statistics()
    ...
    def __getitem__(self, item):
                ...
                if self.init_dates is None:
                    ...
                    lazy_data = self.ds[p].sel(
                        time=slice(self.init_dates[item],
                                   self.init_dates[item]+pd.Timedelta(f"{self.sequence_length*self.timedelta*24}h"))
        ...
        assert prescribed.any()  != None, f"Chunk index: {item} to {item+self.sequence_length}"
