...
from weatherbench2.Weatherbench2_Regions import Region
import xarray as xr
...
def get_adjusting_weights(deg):
    ...
    return weights #weights.unsqueeze(1) #.expand(-1, expand)
...
def get_lat_weights(ds: xr.Dataset) -> xr.DataArray:
  ...
  # weights = ds.lat.copy(data=weights)
   ...
   # Create a new xr.DataArray with the weights
  ...
  #       name='lat_weights'
  #   )
  weights = get_adjusting_weights(2.0).numpy()

  data_array = xr.DataArray(
        data=weights,
        dims=['lat'],  # Dimensions should be a list
        coords={'lat': ds.lat},  # Coordinates are keyed by dimension name
        name='lat_weights'
    )

  return data_array #get_adjusting_weights(2.0).numpy()
