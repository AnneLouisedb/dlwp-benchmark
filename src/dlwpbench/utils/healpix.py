...
class HEALPixLayer(th.nn.Module):
    ...
    def zero_out(self):
        """Zero out the parameters of this module."""
        return zero_module(self)
...
class BARNNHEALPixLayer(ConditionedBlock):
    """
    Pytorch module for applying any base torch Module on a HEALPix tensor. Expects all input/output tensors to have a
    shape [..., 12, H, W], where 12 is the dimension of the faces.
    """
    ...
    def forward(self, x: th.Tensor, alpha=None) -> th.Tensor:
        
        for module in self.layers:
                
            x = module(x, alpha = alpha)

            return x

    def kl(self, alpha):
        for module in self.layers:
            if not isinstance(module, HEALPixLayer):
...
class HEALPixPadding(th.nn.Module):
    ...
    def forward(self, data: th.Tensor) -> th.Tensor:
        ...
        #device = th.device('cpu')  # Specify CPU device
        #data = data.to(device)  # Move input to CPU
        #####
        ...
        del f00, f01, f02, f03, f04, f05, f06, f07, f08, f09, f10, f11
        del p00, p01, p02, p03, p04, p05, p06, p07, p08, p09, p10, p11
