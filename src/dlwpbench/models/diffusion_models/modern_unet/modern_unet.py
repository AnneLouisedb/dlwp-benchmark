...
from utils import HEALPixPadding,ConditionalHEALPixLayer, HEALPixLayer
from abc import ABC, abstractmethod
import math 
from torch.cuda.amp import autocast
from diffusers.schedulers import DDPMScheduler,FlowMatchEulerDiscreteScheduler
from scripts.helper_scripts.evaluation_helper import remap
# Load models for backbone operator
from models import *

def dict_apply(f, *args, **kwargs):
    """
    Apply a function `f` to dictionaries of tensors.

    Args:
        f (callable): Function to apply element-wise.
        args: Positional arguments, which can include dictionaries of tensors or plain tensors.
        kwargs: Keyword arguments, which can include dictionaries of tensors or plain tensors.

    Returns:
        dict: A dictionary where the function `f` has been applied to corresponding keys,
              or a tensor if no dictionaries are present.
    """
    # Extract dictionaries from args and kwargs
    dicts = [a for a in args if isinstance(a, dict)]
    dicts += [v for v in kwargs.values() if isinstance(v, dict)]

    # If no dictionaries are found, apply the function directly
    if not dicts:
        return f(*args, **kwargs)

    # Verify all dictionaries have the same keys
    dict_keys = set(dicts[0].keys())
    for d in dicts[1:]:
        if dict_keys != set(d.keys()):
            raise ValueError("All dictionaries must have the same keys")

    # Apply the function element-wise to each key
    return {
        k: f(
            *[(a[k] if isinstance(a, dict) else a) for a in args],
            **{ki: (vi[k] if isinstance(vi, dict) else vi) for ki, vi in kwargs.items()},
        )
        for k in dict_keys
    }
...
class DiffModernUNet(DiffusionModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # #checkpoint_backbone = th.load('/home/adboer/dlwp-benchmark/src/dlwpbench/outputs/C1T1D01PDErefhpx321239/checkpoints/C1T1D01PDErefhpx321239_best.ckpt')
    ...
    def single_forward(self, constants, prescribed, prognostic, y_noised, time):
        ...
        if y_noised.ndim == 5: # make sure tat the y_noised matches the prognostic in the time dimension
            y_noised = y_noised.expand(-1, prognostic.shape[1], -1, -1, -1) # B, T, C, H, W

        elif y_noised.ndim == 6:
            y_noised = y_noised.expand(-1, prognostic.shape[1], -1, -1, -1, -1) # adding time dimension
    ...
    def diffusion_forward(self, constants, prescribed, prognostic, noise_scheduler, target_shape):
        ...
        for k in noise_scheduler.timesteps:
            ...
            if isinstance(noise_scheduler, FlowMatchEulerDiscreteScheduler):
                ...
                step_index = getattr(noise_scheduler, "_step_index", None)

                def scheduler_step(*args, **kwargs):
                    out = noise_scheduler.step(*args, **kwargs)
                    if hasattr(noise_scheduler, "_step_index"):
                        noise_scheduler._step_index = step_index
                    return out.prev_sample

                scheduler_kwargs = dict(s_churn=0.1, s_tmin=0.0, s_tmax=float("inf"))
                # return prev_sample
                y_noised = scheduler_step(pred, k, y_noised, **scheduler_kwargs)
    ...
    def forward(
        ...
        # [C, C, T1, T2, T3] -> prognostic time dimension
            ...
            print('shape prescribed', prescribed.shape)
            print('shape prognostic', prognostic.shape)
           

            # Slices the last C1 from the tensor for inputting to the model
            ...
            print('output diffusion', out.shape)
           
            if self.mesh == "healpix": out = einops.rearrange(out, "(b f) tc h w -> b tc f h w", b=B, f=F)

            out = prognostic_t[:, -1] + 0.3*out #  since we are training to predict the residual!!
...
class ModernUNetEncoder(th.nn.Module):
    ...
    def __init__(
        ...
        for c_idx in range(len(channels[:-1])):
            ...
            if c_idx > 0: layer.append(HEALPixLayer(layer=th.nn.Conv2d, in_channels=c_in, out_channels=c_in, kernel_size=3, stride = 2, padding=1))
    ...
    def forward(self, x: th.Tensor, emb:th.Tensor) -> list:
        ...
        for layer in self.layers:
            ...
            for module in layer:
                ...
                if isinstance(module,(th.nn.ConvTranspose2d, th.nn.Identity, th.nn.Conv2d, HEALPixLayer)):
                    x = module(x)
                     
                else:
                    x = module(x, emb)

                if not isinstance(module, (AttentionBlock, th.nn.Identity)):
                    outs.append(x)  
...
class ModernUNetDecoder(th.nn.Module):
    ...
    def __init__(
        ...
        self.layers = []
        final_out = hidden_channels[0]  # due to time embedding
        ...
        for c_idx in range(len(hidden_channels)):
            ...
            # need time embedding as well as skip encoder
            c_in_ = 2*hidden_channels[c_idx]  # Skip connection from encoder
